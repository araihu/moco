...
func TestAuthorizationResourceRecognizesOnlyContractRoutes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "audit read", method: http.MethodGet, path: "/internal/v1/audit", resource: "/internal/v1/audit", matches: true},
		...
		{name: "encryption status", method: http.MethodGet, path: "/internal/v1/encryption/status", resource: "/internal/v1/encryption/status", matches: true},
		{name: "encryption status unsupported", method: http.MethodPost, path: "/internal/v1/encryption/status", matches: false},
